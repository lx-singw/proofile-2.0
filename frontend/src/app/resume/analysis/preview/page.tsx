...
import { ArrowLeft, Download, Edit, FileText, TrendingUp, Target, Award, CheckCircle, AlertCircle, FileCheck, Sparkles, Key, Layout, History } from "lucide-react";
...
export default function PublicAnalysisPreviewPage() {
    const [analysis, setAnalysis] = useState<any | null>(null);
    const [loading, setLoading] = useState(true);
    const [showPaywall, setShowPaywall] = useState(false);
    const [paywallTrigger, setPaywallTrigger] = useState<'save' | 'download' | 'apply_improvements'>('apply_improvements');
    const router = useRouter();
    const { user } = useAuth();

    useEffect(() => {
        // If user is logged in, they shouldn't be here (unless testing)
        // But we'll allow it for now.

        const storedAnalysis = localStorage.getItem('publicAnalysis');
        if (storedAnalysis) {
            try {
                setAnalysis(JSON.parse(storedAnalysis));
            } catch (e) {
                console.error("Failed to parse public analysis", e);
    ...
    const handleRestrictedAction = (action: 'save' | 'download' | 'apply_improvements') => {
        ...
        <div className="min-h-screen bg-gradient-to-br from-gray-50 via-white to-green-50 dark:from-gray-900 dark:via-gray-900 dark:to-gray-800">
            <header className="border-b border-gray-200 dark:border-gray-800 bg-white/95 dark:bg-gray-900/95 backdrop-blur-sm sticky top-0 z-50 flex items-center justify-between px-6 py-4">
                ...
                <div className="flex items-center gap-3">
                    <button
                        onClick={() => handleRestrictedAction('download')}
            ...
            <main className="max-w-5xl mx-auto py-12 px-4">
                ...
                {/* AI Refinement Tools */}
                <AIRefinementTools onToolSelect={() => handleRestrictedAction('apply_improvements')} />

                {/* Job Description Matching */}
                <JobMatching resumeId="public" resumeData={analysis} />

                {/* CTA */}
                <div className="text-center mt-8">
                    <button
                        onClick={() => handleRestrictedAction('apply_improvements')}
                        className="px-8 py-3 bg-gradient-to-r from-green-600 to-emerald-600 text-white rounded-xl font-semibold hover:from-green-700 hover:to-emerald-700 transition-all shadow-lg hover:shadow-xl transform hover:scale-105"
                    >
